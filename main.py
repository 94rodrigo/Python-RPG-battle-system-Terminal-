...
player2 = Person("Nicko", 4160, 188, 311, 34, player_spells, player_items)
...
while running:
    ...
    for player in players:
        ...
        if index == 0:
            ...
            player.choose_item()
            item_coice = int(input("Choose item: ")) - 1

            if item_coice == -1:
                continue

            item = player_items[item_coice]["item"]

            if player.items[item_coice]["quantity"] == 0:
                print(bcolors.FAIL + "\nNone left..." + bcolors.ENDC)
                continue
            player.items[item_coice]["quantity"] -= 1
